...
![../_images/Adapter.jpg](img/Adapter.jpg)

类适配器：

![../_images/Adapter_classModel.jpg](img/Adapter_classModel.jpg)

## 1.4\. 时序图

![../_images/seq_Adapter.jpg](img/seq_Adapter.jpg)
...
```
 ...
 |  
```
#include <iostream> #include "Adapter.h" #include "Adaptee.h" #include "Target.h" using namespace std;

int main(int argc, char *argv[])
{
 Adaptee * adaptee  = new Adaptee();  Target * tar = new Adapter(adaptee);  tar->request(); 	
	return 0;
} 
```
...
#include "Target.h" #include "Adaptee.h" class Adapter : public Target
{
...
#include "Adapter.h" Adapter::Adapter(Adaptee * adaptee){
	m_pAdaptee =  adaptee;
}
...
![../_images/Adapter_run.jpg](img/Adapter_run.jpg)
...
*   适配器模式适用情况包括：系统需要使用现有的类，而这些类的接口不符合系统的需要；想要建立一个可以重复使用的类，用于与一些彼此之间没有太大关联的一些类一起工作。 © 版权所有 2014, Colin http://blog.me115.com. 由 [Sphinx](http://sphinx-doc.org/) 1.3.5 创建。