...
![../_images/Facade.jpg](img/Facade.jpg)

## 4.4\. 时序图

![../_images/seq_Facade.jpg](img/seq_Facade.jpg)
...
```
 ...
 |  
```
#include <iostream> #include "Facade.h" using namespace std;

int main(int argc, char *argv[])
{
 Facade fa;  fa.wrapOpration(); 	
	return 0;
} 
```
...
#include "SystemC.h" #include "SystemA.h" #include "SystemB.h" class Facade
{
...
void Facade::wrapOpration(){  m_SystemA->operationA();  m_SystemB->operationB();  m_SystemC->opeartionC(); } 
```
...
![../_images/Facade_run.jpg](img/Facade_run.jpg)
...
*   外观模式适用情况包括：要为一个复杂子系统提供一个简单接口；客户程序与多个子系统之间存在很大的依赖性；在层次化结构中，需要定义系统中每一层的入口，使得层与层之间不直接产生联系。 © 版权所有 2014, Colin http://blog.me115.com. 由 [Sphinx](http://sphinx-doc.org/) 1.3.5 创建。